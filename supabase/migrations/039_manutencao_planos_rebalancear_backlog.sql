...
--
-- Redistribui esse mesmo backlog ao longo de várias semanas, POR ÁREA (cada tela de
-- Programação — Mecânica/Elétrica/Apoio — filtra por área, então o lote é por área, não
-- global), com duas regras combinadas com o usuário:
--   1. No máximo 8 preventivas "grandes" (periodicidade semestral+: 6, 12, 13, 24 meses)
--      por semana por área — perder o prazo de uma anual não é visível de novo por meses
--      (tolerância de atraso é ~1/3 do período), então não pode virar backlog invisível.
--   2. O resto da vaga da semana (até completar 25 no total — mesmo lote já usado no
--      painel de sugestões, LOTE_PREVENTIVAS_POR_SEMANA_NOVO em
--      manutencao-programacao.component.ts) é preenchido com as "pequenas" (< 6 meses).
--
-- Dentro de cada grupo (grandes/pequenas) de cada área, os planos são intercalados
-- proporcionalmente por periodicidade (ex.: não deixa as 45 anuais da Elétrica todas
-- antes das 121 semestrais) e ordenados por código pra determinismo.
--
-- O mapeamento id -> nova data_inicial abaixo foi calculado com esse algoritmo rodado
-- sobre o estado real do backlog (748 planos: 336 Mecânica, 286 Elétrica, 126 Apoio) —
-- por isso os UPDATEs são explícitos por id em vez de uma query só: a lógica de "encher
-- a semana até 8 grandes + resto de pequenas até 25" não é natural de expressar em SQL
-- declarativo puro (precisa de dois cursores avançando em ritmos diferentes). Resultado:
-- Mecânica usa 14 semanas (até 2026-12-21), Elétrica 22 semanas (até 2027-02-15, puxado
-- pelas 169 grandes a 8/semana), Apoio 8 semanas (até 2026-11-09).
--
-- Exceção, por cima do cálculo acima: o teste de disponibilidade do sistema de carvão
-- (Empilhadeira/Retomadeira, equipamento PTPC91, cadastrado uma vez por área) precisa
-- continuar caindo especificamente na semana 39 — pedido explícito do usuário, ver o
...
-- o cálculo acima tenha jogado algum deles mais pra frente (3 dos 6 caíam em
-- outubro/novembro sem isso). Roda por último de propósito, pra sobrepor o resultado
-- geral só nesses seis planos específicos.
UPDATE manutencao_planos SET data_inicial = '2026-09-21' WHERE id = ANY(ARRAY[
